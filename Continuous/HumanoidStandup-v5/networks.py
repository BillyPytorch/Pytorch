...
class RunningMeanStd:
    ...
    def update(self, x):
        ...
        batch_mean = x.mean(dim=0) # dim=0 mean of columns aka for that feature
        # var = sum(each num - mean)**2 / tot_num = the average of the squared distances from the mean. Highlights furhter away values
        batch_var = x.var(dim=0, unbiased=False) # false for entire dataset(population not sample variance)
        batch_count = x.shape[0] # num of observations

        delta = batch_mean - self.mean # distance between two means
        total_count = self.count + batch_count

        new_mean = self.mean + delta * batch_count / total_count # weighted average (new and old mean)

        m_a = self.var * self.count #total old squared deviation from its mean, spread of the group
        m_b = batch_var * batch_count #total new squared deviation from its mean
        ...
        # spread caused by the TWO GROUPS being separated
        # spread_a + spread_b + squared distance between the means * count weight
        # m2 = total spread from TWO GROUPS being separated



        new_var = m2 / total_count # Average squared spread per observation || Average squared distance from the mean
